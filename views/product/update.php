...
$(function(){
	...
	$('#list_imgs').uploadify({
        ...
        'onUploadSuccess': function (file, data, response) {
	        var res = eval('(' + data + ')');
	        var span = "<span style='border:1px solid #aaa; padding:5px;float:left;width:212px;margin-right:20px;margin-bottom:10px;'>" +
...
function chgNeedSend(obj) {
	$("#warn_number_input").attr('disabled', $(obj).val() != 1);
}
</script>
		...
		<div class="col-xs-12">
			<?php $form = ActiveForm::begin(['options' => ['class' => 'form-horizontal']])?>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<?=Html::dropDownList('ProductForm[grounding]', $productForm->grounding, ['否', '是'])?>
					</div>
					<div class="col-sm-3">
						<?php if ($productForm->hasErrors('need_send')):?>
							<span class="block error"><?=Html::error($productForm, 'grounding')?></span>
						<?php endif;?>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<?=Html::dropDownList('ProductForm[need_send]', $productForm->need_send, ['否', '是'], ['onclick' => 'chgNeedSend(this)'])?>
					</div>
					<div class="col-sm-3">
						<?php if ($productForm->hasErrors('need_send')):?>
